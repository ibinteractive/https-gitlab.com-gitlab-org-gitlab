...
module Gitlab
  module Database
    module MigrationHelpers
      ...
      # - It tracks the scheduled background jobs through the use of Gitlab::Database::BackgroundMigrationJob
      ...
      def backfill_conversion_of_integer_to_bigint(
        ...
        sub_batch_size: 1000,
        interval: 2.minutes,
        sample_n_first_batches: nil
      )

        if transaction_open?
          raise 'backfill_conversion_of_integer_to_bigint can not be run inside a transaction'
        end
        ...
        source_relation = define_batchable_model(table)

        if sample_n_first_batches.is_a?(Integer)
          max_primary_key_value = source_relation.select(primary_key).order(primary_key)
            .offset(batch_size * sample_n_first_batches).limit(1).first

          unless max_primary_key_value.nil?
            quoted_primary_key = quote_column_name(primary_key)
            source_relation = source_relation.where("#{quoted_primary_key} < ?", max_primary_key_value)
          end
        end
