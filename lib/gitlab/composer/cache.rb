    ...
    class Cache
      ...
      def execute
        Packages::Composer::Metadatum.transaction do # rubocop: disable CodeReuse/ActiveRecord
          ...
          if locked_package_metadata.any?
            ...
            locked_package_metadata.update!(version_cache_sha: versions_index.sha)
          elsif @last_page_sha
      ...
      def mark_pages_for_delete(shas)
        Packages::Composer::CacheFile
          ...
          .with_sha(shas)
          .update(delete_at: 1.day.from_now)
      end

      def create_cache_page!
        Packages::Composer::CacheFile
          .with_namespace(@project.namespace)
          .with_sha(versions_index.sha)
          .safe_find_or_create_by!(namespace_id: @project.namespace_id, file_sha256: versions_index.sha) do |cache_file|
            file = Tempfile.new("composer-cache-#{versions_index.sha}.json", Dir.tmpdir)

            file.write(versions_index.to_json)
            file.rewind
      ...
      def shas_to_delete
        locked_package_metadata
          ...
          .reject { |sha| sha == versions_index.sha }
          .compact
      end
    end
  end
